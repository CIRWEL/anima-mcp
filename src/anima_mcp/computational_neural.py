...
- Disk I/O throughput → Theta (background data movement)
...
class ComputationalNeuralState:
    ...
    theta: float   # 0-1: Background I/O throughput (disk read/write bytes)
...
class ComputationalNeuralSensor:
    ...
    - Theta: Disk I/O bytes per second (background data movement)
    ...
    def get_neural_state(self, cpu_percent: Optional[float] = None,
        ...
        # === THETA: Background I/O throughput (disk read+write bytes/sec) ===
        # Measures actual data movement — "subconscious" processing, file saves, logs
        theta = 0.0
        try:
            ...
            if disk_io and self._last_disk_io is not None and dt > 0:
                ...
                # Normalize: ~1MB/s is moderate, ~10MB/s is heavy
                theta = min(1.0, bytes_per_sec / (10 * 1024 * 1024))
            if disk_io:
