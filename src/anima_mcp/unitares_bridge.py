...
async def resolve_caller_identity(
    ...
    """
    Resolve the calling agent's identity from UNITARES.

    Used by anima tools (lumen_qa, post_message) to get the verified
    agent name instead of trusting user-provided agent_name parameter.

    Args:
        unitares_url: UNITARES server URL (uses env var if None)
        session_id: Session ID to resolve (uses default if None)
        timeout: Request timeout (short since this is a lookup)

    Returns:
        Agent's display_name/label if found, None otherwise
    """
    url = unitares_url or os.environ.get("UNITARES_URL")
    if not url:
        return None
    ...
    try:
        ...
        # Build MCP request for identity()
        mcp_request = {
            "jsonrpc": "2.0",
            "id": 1,
            "method": "tools/call",
        ...
        # Quick lookup with short timeout
                ...
                if response.status == 200:
                    ...
                    if result and "result" in result:
                        ...
                        if "content" in content and content["content"]:
                            try:
                                inner = json.loads(content["content"][0].get("text", "{}"))
                                # Return display_name or label
                                return inner.get("display_name") or inner.get("label")
