    ...
    def drift_focus(
        ...
        if state.zone_remaining > 0:
            ...
            if dist > state.zone_radius * 0.7:
                ...
                focus_x += dx / dist * pull + random.gauss(0, 2.0)
                focus_y += dy / dist * pull + random.gauss(0, 2.0)
            else:
                # Random wander within zone
                focus_x += random.gauss(0, 3.0)
                focus_y += random.gauss(0, 3.0)
            ...
            # Zone size varies with coherence: high C = tighter zones
            state.zone_radius = random.uniform(15, 50 - 20 * C)
