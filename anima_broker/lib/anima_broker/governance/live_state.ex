defmodule AnimaBroker.Governance.LiveState do
  ...
  Returns canonical `body_anima`, its deprecated `anima` alias, and `readings`
  only when the envelope is fresh (same staleness contract as the Python
  side's env-sensor consumer, default 30s); otherwise
  `{:error, :stale | :missing | :invalid}`.
  """
  ...
  defp normalize_data(data) do
    body_anima = Map.get(data, "body_anima") || Map.get(data, "anima")
    readings = Map.get(data, "readings")
    body_keys = ["warmth", "clarity", "stability", "presence"]

    valid_body? =
      is_map(body_anima) and
        Enum.all?(body_keys, fn key ->
          value = Map.get(body_anima, key)
          is_number(value) and value >= 0.0 and value <= 1.0
        end)

    if valid_body? and is_map(readings) do
      {:ok,
       %{
